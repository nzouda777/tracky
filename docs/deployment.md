...
| `/apps/track-order` redirects to `/proxy/track-order` on the merchant's domain, which 404s | The App Proxy URL has a trailing slash, so the app answered with a 308 whose relative `Location` the browser resolved against the storefront | Remove the trailing slash in the Partner dashboard. `skipTrailingSlashRedirect` in `next.config.ts` stops the app redirecting at all, so a deployment carrying it tolerates either spelling |
